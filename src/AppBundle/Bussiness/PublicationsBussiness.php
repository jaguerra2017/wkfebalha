...
class PublicationsBussiness
{
    ...
    public function loadInitialsData($parametersCollection)
    {
        try{
            $initialsData = array();
            $initialsData['publicationsDataCollection'] = $this->getPublicationsList($parametersCollection);
    ...
    public function getPublicationsList($parametersCollection)
    {
        try{
            ...
            if(isset($parametersCollection['singleResult'])){
                ...
                $idsCollection = array();
                $idsCollection[0] = $parametersCollection['publicationId'];
                $idsCollection = implode(',', $idsCollection);
                $parametersCollection['idsCollection'] = $idsCollection;
                $publicationsCollection = $this->em->getRepository('AppBundle:GenericPost')->getGenericPostsFullData($parametersCollection);
            }
            else{
                $publicationsCollection = $this->em->getRepository('AppBundle:GenericPost')->getGenericPostsBasicData($parametersCollection);
            }
            if(isset($publicationsCollection[0])){
                foreach($publicationsCollection as $key=>$publication){
                    $canEdit = 1;
                    $canDelete = 1;
                    $publicationsCollection[$key]['canEdit'] = $canEdit;
                    $publicationsCollection[$key]['canDelete'] = $canDelete;

                    /*handling Post Status*/
                    $objGenericPost = $this->em->getRepository('AppBundle:GenericPost')->find($publication['id']);
                    ...
                    if(isset($objPostStatus)){
                        $publicationsCollection[$key]['post_status_name'] = $objPostStatus->getNomenclature()->getName();
                        if(isset($parametersCollection['singleResult'])){
                            $publicationsCollection[$key]['post_status_id'] = $objPostStatus->getNomenclature()->getId();
                        }
                    }

                    /*handling dates*/
                    $publicationsCollection[$key]['created_date'] = date_format($publication['created_date'],'d/m/Y');
                    if($publication['modified_date'] != null){
                        $publicationsCollection[$key]['modified_date'] = date_format($publication['modified_date'],'d/m/Y');
                    }
                    if($publication['published_date'] != null){
                        $publicationsCollection[$key]['published_date'] = date_format($publication['published_date'],'d/m/Y');
                    }

                    /*handling featured image urls*/
                    if($publication['have_featured_image'] == true){
                        $objMediaImage = $this->em->getRepository('AppBundle:MediaImage')->find($publication['featured_image_id']);
                        ...
                        if($objSharedFileFinderBussiness->getExistenceFilteredUploadedImage(array(
                            ...
                            'image_name' => $publication['featured_image_name'],
                            'image_extension' => $featured_image_extension,
                            'just_check' => true,
                            'just_web_filtered_url' => false
                        ))){
                            $publicationsCollection[$key]['web_filtered_list_featured_image_mini_thumbnail_url'] = $objSharedFileFinderBussiness->getExistenceFilteredUploadedImage(array(
                                'filter_name' => 'list_featured_image_mini_thumbnail',
                                'image_name' => $publication['featured_image_name'],
                                'image_extension' => $featured_image_extension,
                                'just_check' => false,
                            ...
                            $publicationsCollection[$key]['web_filtered_list_featured_image_mini_thumbnail_url'] = $parametersCollection['imagineCacheManager']->getBrowserPath($publication['featured_image_url'], 'list_featured_image_mini_thumbnail');
                        }
                        /*Grid List thumbnail*/
                        if($objSharedFileFinderBussiness->getExistenceFilteredUploadedImage(array(
                            ...
                            $publicationsCollection[$key]['web_filtered_grid_featured_image_thumbnail_url'] = $objSharedFileFinderBussiness->getExistenceFilteredUploadedImage(array(
                                'filter_name' => 'grid_featured_image_thumbnail',
                                'image_name' => $publication['featured_image_name'],
                                'image_extension' => $featured_image_extension,
                                'just_check' => false,
                            ...
                            $publicationsCollection[$key]['web_filtered_grid_featured_image_thumbnail_url'] = $parametersCollection['imagineCacheManager']->getBrowserPath($publication['featured_image_url'], 'grid_featured_image_thumbnail');
                        }
                        ...
                        if(isset($parametersCollection['singleResult']) && $parametersCollection['singleResult'] == true){
                            if($objSharedFileFinderBussiness->getExistenceFilteredUploadedImage(array(
                                ...
                                $publicationsCollection[$key]['web_filtered_single_post_feature_image_url'] = $objSharedFileFinderBussiness->getExistenceFilteredUploadedImage(array(
                                    'filter_name' => 'single_post_featured_image',
                                    'image_name' => $publication['featured_image_name'],
                                    'image_extension' => $featured_image_extension,
                                    'just_check' => false,
                                ...
                                $publicationsCollection[$key]['web_filtered_single_post_feature_image_url'] = $parametersCollection['imagineCacheManager']->getBrowserPath($publication['featured_image_url'], 'single_post_featured_image');
                            }
                        }
                    }
                    ...
                    if($this->container != null){
                        ...
                        $publicationsCollection[$key]['url'] = $siteDomain.'/es/publicaciones/'.$publication['url_slug_es'];
                    }
                    ...
                    $publicationsCollection[$key]['total_comments'] = $totalComments;

                    /*handling data for Single Result*/
                    if(isset($parametersCollection['singleResult']) && $parametersCollection['singleResult'] == true){
                        /*handling Categories*/
                        $genericPostsId = array();
                        $genericPostsId[0] = $publication['id'];
                        ...
                        $publicationsCollection[$key]['categoriesCollection'] = $categoriesCollection;
                    }
                }
            }

            if(isset($parametersCollection['singleResult']) && $parametersCollection['singleResult'] == true
            && isset($publicationsCollection[0])){
                return $publicationsCollection[0];
            }
    ...
    public function savePublicationData($parametersCollection){
        try{
            ...
            if(isset($objGenericPost)){
                if($parametersCollection['isCreating'] == true ||
                    ...
                    $message = 'Ya existe una publicación con ese nombre.';
            ...
            /*persisting relation Post Status - Publication */
            ...
            /*Handling relation Taxonomy - Publication*/
            ...
            return $this->returnResponse(array('success'=>1,'message'=>$message, 'publicationId'=>$objGenericPost->getId()));
        }
        catch(\Exception $e){
            throw new \Exception($e);
        }
    ...
    public function deletePublicationsData($parametersCollection){
        try{
            $message = 'Datos guardados.';
            if(isset($parametersCollection['publicationsId'][0])) {
                $objCommentBussiness = new CommentsBussiness($this->em);
                $objCommentBussiness->updateCommentsPending(null, $parametersCollection['publicationsId']);

                $idsCollection = implode(',',$parametersCollection['publicationsId']);
